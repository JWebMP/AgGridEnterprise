...
- Library name: AgGridEnterprise
- Current/new version: Phase 2 (Dec 2, 2025)
- Repository URL / path: https://github.com/JWebMP/AgGridEnterprise.git
- Rules Repository Location: rules/ (Git submodule)
- Short description: The JWebMP Plugin and extension for AG Grid to enable Enterprise Functionality
- Type:
...
- Component/topic areas (list): 
  ...
  - Row Grouping (columnDef.rowGroup / rowGroupIndex / keyCreator / valueFormatter)
...
- **Sandbox awareness / Pact note** — Record in PACT that host artifacts stay at the repo root or `docs/`. When a release is finalized, update the canonical rules under `rules/frontend/jwebmp/aggrid-enterprise` (the ruleset this library maintains) and limit writes inside the Rules Repository to `rules/frontend/jwebmp/aggrid-enterprise`; never modify other directories in the rules repo.
...
- Frontend (Standard):
  ...
  - JWebMP AgGrid (community): rules/generative/frontend/jwebmp/aggrid/README.md
  - JWebMP AgGridEnterprise: (This library's topic index)
  ...
  - Frontend (Reactive):
    ...
    - Next.js: rules/generative/frontend/nextjs/README.md
...
- AG Grid Enterprise (Grid component library)
  - Ensure parent README index exists and includes components/features such as charts.rules.md, row-grouping.rules.md, range-selection.rules.md, server-side-row-model.rules.md, dynamic-series-coloring.rules.md, etc.
  - Provide JWebMP Java wrapper rules that wrap/extend underlying AG Grid Enterprise contracts and options.
  - For integration with host projects, document the fluent API pattern (CRTP), module registration, and boot-time wiring via GuicedEE `IPageConfigurator`.
  - Reference the dynamic series coloring feature with comprehensive examples covering all 5 strategies (SOLID, VALUE_GRADIENT, VALUE_RANGE, POSITIVE_NEGATIVE, CUSTOM_CALLBACK).
  - If AgCharts is used alongside, clarify integration points and any option conflicts.
- JWebMP wrappers
